...
namespace UPS.AddressValidation.Application.Validators;

public sealed class ValidateAddressCommandValidator : AbstractValidator<DTOs.ValidateAddressCommand>
{
    public ValidateAddressCommandValidator()
    {
        RuleFor(x => x.AddressLines)
            .NotNull()
            .Must(lines => lines.Count is >= 1 and <= 3)
        ...
        RuleFor(x => x.CountryCode)
            ...
            .Length(2)
            .Matches("^[A-Za-z]{2}$")
            .WithMessage("CountryCode must be a 2-letter ISO country/territory code.");

        RuleFor(x => x.RequestOption)
